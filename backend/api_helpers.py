...
class APIHelpers:
        ...
        try:
            with self.get_db_connection() as conn:
                ...
                photo = {
                    ...
                    "path": result[1],
                    "timestamp": result[2],  # Keep as integer
                    "objects": objects
                }
                ...
                for face_row in cursor.fetchall():
                    # Safely parse bbox JSON
                    bbox = []
                    if face_row[1]:  # If bbox field is not None or empty
                        try:
                            bbox = json.loads(face_row[1])
                        except (json.JSONDecodeError, TypeError):
                            logger.warning(f"Invalid JSON in bbox field for face in photo {photo_id}: {face_row[1]}")
                            bbox = []
    ...
    def get_photo_relationships(self, photo_id: str) -> List[Dict[str, Any]]:
